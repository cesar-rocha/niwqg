...
wavemodel = CoupledModel.Model(L=L,nx=nx, tmax = tmax,dt = dt, twrite=20,
                    nu4=0,mu=mu,nu4w=0,nu=0,nuw=0,muw=4*mu, use_filter=True,save_to_disk=True,
...
E = (wavemodel.sigma_w/wavemodel.muw)**2 / 2.
prediction =  ((wavemodel.sigma_w/wavemodel.muw)*(1-np.exp(-wavemodel.muw*time)))**2 / 2

# plot energy
fig = plt.figure(figsize=(8.5,4.))
ax = fig.add_subplot(111)
plt.plot(time*wavemodel.muw,KE_niw/E)
plt.plot(time*wavemodel.muw,prediction/E)
plt.xlabel(r"Time $[t\,\,\mu]$")
plt.ylabel(r"Wave kinetic energy $[\mathcal{A} \,\, 2 \gamma^2/\sigma_w^2]$")
